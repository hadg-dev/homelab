...
def wait_for_vm(name: str, retries: int = 30):
...
def check_cluster_tools():
    console.print("[bold]Checking cluster dependencies...[/bold]")

    try:
        subprocess.run("multipass version", shell=True, check=True, stdout=subprocess.DEVNULL)
        subprocess.run("multipass launch --help", shell=True, check=True, stdout=subprocess.DEVNULL)
        console.print("[green]✔ multipass found[/green]")
    except subprocess.CalledProcessError:
        console.print("[red]Missing dependency: multipass[/red]")
        console.print("Install with: brew install multipass")
        return False
...
def create_vms():
    ...
    if not vm_exists("k8s-ctrl"):
        run(
            "multipass launch 24.04 --name k8s-ctrl --cpus 2 --memory 4G --disk 20G"
        )
    else:
    ...
    if not vm_exists("k8s-worker-1"):
        run(
            "multipass launch 24.04 --name k8s-worker-1 --cpus 2 --memory 4G --disk 50G"
        )
    else:
    ...
    if not vm_exists("k8s-worker-2"):
        run(
            "multipass launch 24.04 --name k8s-worker-2 --cpus 2 --memory 4G --disk 50G"
        )
    else:
    ...
    wait_for_vm("k8s-worker-2")
    state.set("vms_created", True)
...
def join_worker():
    ...
    run(
        f"multipass exec k8s-worker-1 -- bash -c "
        f"\"curl -sfL https://get.k3s.io | K3S_URL=https://$(multipass info k8s-ctrl | grep IPv4 | awk '{{print $2}}'):6443 K3S_TOKEN={token} sh -\""
    )

    run(
        f"multipass exec k8s-worker-2 -- bash -c "
        f"\"curl -sfL https://get.k3s.io | K3S_URL=https://$(multipass info k8s-ctrl | grep IPv4 | awk '{{print $2}}'):6443 K3S_TOKEN={token} sh -\""
    )

    state.set("worker_joined", True)

def configure_k3s_kubeconfig_permissions():
    console.print(
        "[bold yellow]Configuring kubectl access in control plane...[/bold yellow]"
    )
...
def setup_local_kubeconfig():
    ...
    run(
        f"""multipass exec k8s-ctrl -- sudo cat /etc/rancher/k3s/k3s.yaml \
| sed 's/127.0.0.1/{ctrl_ip}/' \
> ~/.kube/config"""
    )
